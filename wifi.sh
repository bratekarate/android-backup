...
gpg -d WifiConfigStore.xml.gpg >/tmp/WifiConfigStore.xml && for CFG in /tmp/WifiConfigStore.xml WifiConfigStore.xml.encrypted-checksum; do adb push "$CFG" /sdcard; done && rm /tmp/WifiConfigStore.xml && adb shell "su -c 'cd /data/misc/wifi && mv WifiConfigStore.xml WifiConfigStore.xml.bak && mv /sdcard/WifiConfigStore.xml* . && chown 1000:1000 WifiConfigStore.xml*'" && adb reboot

# setup correct magisk channel
adb shell "su -i -c \"sed 's|\(custom_channel.*>\).*\(<\)|\1https://raw.githubusercontent.com/topjohnwu/magisk_files/63555595ffa9b079f3a411dd2c00a80a3d985ccc/stable.json\2|g' /data/user_de/0/com.topjohnwu.magisk/shared_prefs/com.topjohnwu.magisk_preferences.xml\""
